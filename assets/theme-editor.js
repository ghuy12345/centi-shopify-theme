...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateShopPay() {
        ...
        // Remove unwanted text nodes but keep £ price + Shop Pay logo
        content.childNodes.forEach(node => {

            if (node.nodeType === Node.TEXT_NODE) {

                node.textContent = node.textContent
                    .replace(/Pay in\s+\d+\s+interest[- ]free\s+instalments?\s+of\s+/i, "")
    ...
    // Re-run when Shopify updates the widget (variant change)
    const observer = new MutationObserver(updateShopPay);
